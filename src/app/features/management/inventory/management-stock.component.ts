...
import { StockAdjustDialogComponent } from '../../store/stock/stock-adjust-dialog.component';
import { InventoryFormModalComponent, ProductFormData } from './inventory-form-modal/inventory-form-modal.component';
...
@Component({
  ...
  templateUrl: './management-stock.component.html',
  styleUrl: './management-stock.component.scss',
})
export class ManagementStockComponent implements OnInit {
  ...
  readonly filteredProducts = computed(() => {
    ...
    // Target menu-stock items for management stock management
    let products = this.store.products().filter((p) => p.productType === 'menu-stock');
    ...
    ref.afterClosed().subscribe((product) => {
      if (product) {
        this.store.addProduct(product).subscribe({
          next: () => {
            this.sweetAlert.success(`"${product.name}" added to stock`);
          },
          error: (error: any) => {
  ...
  openAdjustDialog(product: Product) {
    ...
    ref.afterClosed().subscribe((delta: number | undefined) => {
      if (delta) {
        ...
        const msg =
          mode === 'add'
            ? `Added ${delta} ${product.unit || 'units'} to ${product.name}`
            : `Deducted ${Math.abs(delta)} ${product.unit || 'units'} from ${product.name}`;
